...
def _is_under_include(path: Path, root: Path, include: list[str]) -> bool:
    """Return True if path lives under at least one of the include subpaths (relative to root).

    include entries are relative to root (e.g. "Vault", "_pipeline/drafts"). Leading/trailing
    slashes are stripped. An empty include list means "no restriction" — caller should skip
    calling this helper in that case.
    """
    try:
        rel = str(path.relative_to(root))
    except ValueError:
    ...
    for entry in include:
        ...
        if not entry:
            continue
        # Match "Vault" against "Vault/foo.md" or "Vault" itself
        if rel == entry or rel.startswith(entry + "/"):
            return True
    return False


def detect(root: Path, *, follow_symlinks: bool = False, include: list[str] | None = None) -> dict:
    """Walk root and classify files. Returns a detection dict (see bottom of function).

    If ``include`` is given, the walk is scoped: only files living under one of the listed
    subpaths (relative to ``root``) are kept. Example: ``include=["Vault", "_pipeline", "_meta"]``
    scans a brain folder but ignores sibling directories like `_raw/` or `graphify-out/` that
    don't belong in the graph. ``graphify-out/memory/`` is always scanned (query-result feedback
    loop) regardless of ``include``.
    """
    ...
    include = [e for e in (include or []) if e and e.strip()]
    ...
    for p in all_files:
        ...
        if _is_ignored(p, root, ignore_patterns):
            continue
        # Positive include scoping: if include is set, keep only files under an included subpath.
        # Files inside memory_dir (graphify-out/memory) bypass this filter so the feedback loop
        # always flows back into the graph.
        if include and not in_memory and not _is_under_include(p, root, include):
            continue
        if _is_sensitive(p):
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - install python-docx, python-pptx, openpyxl]")
                continue
...
def detect_incremental(root: Path, manifest_path: str = _MANIFEST_PATH, *, include: list[str] | None = None) -> dict:
    ...
    ``include`` is forwarded to ``detect()`` to keep incremental scans scoped the same way.
    """
    full = detect(root, include=include)
