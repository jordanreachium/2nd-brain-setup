...
**`--update` flag:** If `--update` was in the invocation arguments AND `graphify-out/graph.json` already exists (a baseline to diff against), this is an incremental rebuild. Run Steps 1 and 2 to verify the install + detect files, then jump to the **"## For --update (incremental re-extraction)"** section below (search for that heading) — DO NOT run Steps 3–8 of the main flow in this case. The `--update` section handles its own merge + downstream pipeline. If `--update` was passed but `graphify-out/graph.json` does NOT exist (first-time run), ignore `--update` and follow the full main flow.
...
# Write interpreter path + UTF-8 mode for all subsequent steps.
# -X utf8 is REQUIRED on Windows (locale.getpreferredencoding() is cp1252 there,
# which can't encode '→' etc. in GRAPH_REPORT.md). No-op on Linux/macOS (already UTF-8).
mkdir -p graphify-out
"$PYTHON" -c "import sys; open('graphify-out/.graphify_python', 'w').write(sys.executable + ' -X utf8')"
```
...
**Optional `include` scoping.** `detect()` accepts an `include=[...]` kwarg that scopes the walk to one or more subpaths relative to `INPUT_PATH`. Use it when a caller (e.g. `/update-brain <brain> graph`) wants to graphify only specific folders inside a larger tree and skip siblings like `_raw/` or `graphify-out/`. Example:

```python
result = detect(Path('INPUT_PATH'), include=['Vault', '_pipeline', '_meta'])
```

Only files living under one of the listed subpaths are kept. `graphify-out/memory/` is always scanned (query-result feedback loop) regardless of `include`. `detect_incremental(..., include=[...])` accepts the same kwarg. Negative filtering via `.graphifyignore` still applies on top.
...
```bash
python3 -X utf8 -m graphify.serve graphify-out/graph.json
```
...
```bash
python3 -X utf8 -m graphify.watch INPUT_PATH --debounce 3
```
